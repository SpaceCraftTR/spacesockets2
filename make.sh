...
mkdir build
g++ example/ExampleUDPServer.cpp -o build/udp_server_application 
if [ $? -eq 0 ]
then
...
g++ example/ExampleUDPClient.cpp -o build/udp_client_application
if [ $? -eq 0 ]
then
...
g++ example/SpacePing.cpp -o build/spaceping
if [ $? -eq 0 ]
then
...
g++ example/ExampleTCPMessagingClient.cpp -o build/example_tcp_messaging_client
if [ $? -eq 0 ]
then
...
g++ example/ExampleTCPMessagingServer.cpp -o build/example_tcp_messaging_server
if [ $? -eq 0 ]
then
